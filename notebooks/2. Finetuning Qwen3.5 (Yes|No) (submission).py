...
STATE_FILE = BASE_DIR / f"submission_finetuning_{CATEGORY}__yes_no_state.json"
...
# %%
completed_tasks = set()
yes_no_state = defaultdict(lambda: defaultdict(dict))

if STATE_FILE.exists():
    with open(STATE_FILE, "r") as f:
        saved_yes_no_state = json.load(f)

    # Reconstruct completed_tasks and yes_no_state
    for s_id, sub_figs in saved_yes_no_state.items():
        for s_fig, questions in sub_figs.items():
            ...
            if isinstance(questions, list):
                for q_obj in questions:
                    ...
                    yes_no_state[s_id][s_fig][q_text] = q_obj["answer"]
            else:
                # Handle the new dictionary format
                for q_text, ans_text in questions.items():
                    task_id = f"{s_id}::{s_fig}::{q_text}"
                    completed_tasks.add(task_id)
                    yes_no_state[s_id][s_fig][q_text] = ans_text

    print(f"Loaded tracking state from {STATE_FILE}.")
    print(f"Found {len(completed_tasks)} previously completed yes_no tasks.")
...
for data_item in pbar:
    ...
    # 1. Update the Yes/No-Specific Tracking State
    yes_no_state[sample_id][sub_fig][question_text] = generated
    ...
    target_ans = next(
        (
            ...
            and ans.get("answer_type") == "Yes/No"
        ),
        None,
    )
    ...
    if processed_cnt % 5 == 0:
        ...
        with open(STATE_FILE, "w") as f:
            json.dump(yes_no_state, f, indent=4)
