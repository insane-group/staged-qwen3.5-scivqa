...
def clean_answer(raw_answer: str) -> tuple[str, bool]:
    ...
    for item in raw_elements:
        ...
        # 3. Targeted Bullet/Number Removal
        item = re.sub(
        ...
        # 5 & 6. Filter out junk artifacts, trailing full stops, AND hanging commas
        item = item.strip(" -:.,")

        if item:
            cleaned_elements.append(item)

    # 5. Join as a clean, comma-separated string
    return ", ".join(cleaned_elements), len(cleaned_elements) > 0
